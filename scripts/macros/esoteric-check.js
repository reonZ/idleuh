...
export function esotericCheck(event, actor) {
    ...
    if (!actor || !actor.isOwner || targets.size !== 1 || !target.actor) {
        ui.notifications.warn('You must have an owned token selected and target another one.')
        return
    }
